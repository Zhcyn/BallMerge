...
class Puzzle : SKNode {
    ...
    private func createSquareOne(x: Int, y: Int) -> SKShapeNode {
        let square = SKShapeNode(rectOf: CGSize(width: scale/2, height: scale/2), cornerRadius: 0)
...
//        square.fillColor = ColorPallete.SQUARE_ONE
        ...
        let node = SKSpriteNode.init(imageNamed: "football")
        node.name = "das"
        node.size = CGSize(width: scale/2, height: scale/2)
        
        square.addChild(node)
        
//        square.fillColor = SKColor.init(patternImage: UIImage(named: "AppLogo")!)
//        let square1 = SKSpriteNode.init(fileNamed: <#T##String#>, securelyWithClasses: <#T##Set<AnyHashable>#>)
...
//        square.fillColor = ColorPallete.SQUARE_TWO
        ...
        let node = SKSpriteNode.init(imageNamed: "bsaketball")
        node.name = "das"
        node.size = CGSize(width: scale/2, height: scale/2)
...
//        square.fillColor = ColorPallete.SQUARE_TWO
//        square.strokeColor = ColorPallete.SQUARE_TWO.darker(by: 50)
        square.strokeColor = ColorPallete.TRIANGLE.darker(by: 50)
        let node = SKSpriteNode.init(imageNamed: "volleyball")
        node.name = "das"
        node.size = CGSize(width: scale/2, height: scale/2)
...
//        return circle
        ...
        square.position = tilePositionOnScreen(x: x, y: y)
//        square.fillColor = ColorPallete.SQUARE_TWO
        square.strokeColor = ColorPallete.CIRCLE.darker(by: 50)
